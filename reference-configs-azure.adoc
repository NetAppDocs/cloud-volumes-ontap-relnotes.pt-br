...
. Para obter informações sobre os tipos de disco suportados para implantações de HA em zonas de disponibilidade únicas e múltiplas, consulte  https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/reference-default-configs.html#azure-ha-pair["Azure (par de HA)"^] .
