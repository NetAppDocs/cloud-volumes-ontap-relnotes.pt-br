...
Recursos e aprimoramentos adicionais também foram introduzidos na versão mais recente do NetApp Console. Para mais informações, veja as novidades em https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html["Cloud Volumes ONTAP"^] gerenciado no Console.



== 9.17.1 P1 (04 de novembro de 2025)

O patch Cloud Volumes ONTAP 9.17.1 P1 já está disponível para implantação e atualização no Azure e no Google Cloud. No entanto, esta versão não está disponível para implantação e atualização na AWS. O Console solicitará que você atualize seus sistemas existentes para esta versão de patch.
